...
8. If shipping a package, run the correct `npm run release:<patch|minor|major> -- --learning "..."` command. Pass `--publish` only when intentionally publishing packages, and pass `--push` only when intentionally pushing `main` and the release tag.
